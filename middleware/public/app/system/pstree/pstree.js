angular.module('nodeadmin.system.pstree', ['ngSanitize'])
.controller('PstreeController', ['$scope', 'System', '$sce', 'AlertCenter', 
function ($scope, System, $sce, AlertCenter) {
  ...
  var colorDependencies = function(string) {
    ...
    $scope.pstree = $sce.trustAsHtml('<div>' + replacedString.join('</div><div>') + '</div>');
  ...
  getPstree = function() {
    ...
    System.getPstree()
    .then(function(resp) {
      var pstree = resp.data.stdout;
      colorDependencies(pstree);
    ...
    .catch(function(err) {
      // Allow for error displaying on pstree page
